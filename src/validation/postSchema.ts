...
export const registerSchema: ZodType<UserRegisterTypes> = object({
  username: string()
    .min(3, { message: "This field required at least  3 characters" })
  ...
  fullname: string()
    ...
    .regex(new RegExp(/^[a-zA-Z0-9]+$/), { message: "Full name is invalid" }),
  email: string()
    .email()
    .min(5, { message: "This field required at least 5 characters" })
    ...
    .toLowerCase(),
  password: string()
    .min(6, { message: "This field required at least 6 characters" })
